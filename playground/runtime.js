...
const handleParallaxElement = (el, scrollY, windowHeight, speed) => {
    ...
    if (rectTop <= windowHeight && (rectTop + rectHeight) >= 0) {
        ...
        if (rangeStart > 0 || rangeEnd < 100) {
            if (normalizedPercentage * 100 < rangeStart)
                normalizedPercentage = 0;
            else if (normalizedPercentage * 100 > rangeEnd)
        ...
        if (el.classList.contains('scroll-parallax-track-container')) {
            const trackX = el.querySelector('.scroll-parallax-track-x');
            if (trackX) {
            ...
            const trackY = el.querySelector('.scroll-parallax-track-y');
            if (trackY) {
                let trackPercentage = -rectTop / (rectHeight - windowHeight);
                trackPercentage = Math.max(0, Math.min(1, trackPercentage));
...
const observeElements = (config) => {
    ...
    const revealElements = document.querySelectorAll('[class*="scroll-reveal"], [class*="scroll-fade"], [class*="scroll-slide"], [class*="scroll-scale"], [class*="scroll-rotate"], [class*="scroll-flip"], [class*="scroll-blur"], [class*="scroll-text-"], [class*="scroll-image-"], [class*="-scroll"], [class*="text-split-"]');
    ...
    horizontalContainers.forEach((container) => {
        ...
        const xChildren = container.querySelectorAll('[class*="scroll-reveal"], [class*="scroll-fade"], [class*="scroll-slide"], [class*="scroll-scale"], [class*="scroll-rotate"], [class*="scroll-flip"], [class*="scroll-blur"], [class*="scroll-text-"]');
    ...
    snapContainers.forEach((container) => {
        ...
        const slides = Array.from(container.querySelectorAll('.snap-start'));
            ...
            entries.forEach((entry) => {
                if (entry.isIntersecting) {
                    ...
                    if (indicatorWrapper) {
                        ...
                        const dotContainer = indicatorWrapper.firstChild;
                        if (dotContainer) {
    ...
    parallaxElements = Array.from(document.querySelectorAll('[class*="scroll-parallax"], [class*="parallax-layer"], .scroll-parallax-track-container'));
    progressElements = Array.from(document.querySelectorAll('[class*="scroll-progress"]'));
};
export const initScrollEffects = (options = {}) => {
