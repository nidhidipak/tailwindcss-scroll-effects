...
export const splitTextNode = (node: Text, splitType: 'chars' | 'words'): DocumentFragment => {
...
export const recursiveSplit = (element: HTMLElement, splitType: 'chars' | 'words') => {
  const childNodes = Array.from(element.childNodes);
  
  childNodes.forEach((node) => {
    if (node.nodeType === Node.TEXT_NODE) {
      const fragment = splitTextNode(node as Text, splitType);
...
export const initTextSplitting = () => {
  ...
  charsElements.forEach((el) => {
    ...
    recursiveSplit(el as HTMLElement, 'chars');
    (el as HTMLElement).style.setProperty('--tw-total-splits', `${charIndex}`);
  });

  const wordsElements = document.querySelectorAll('.text-split-words');
  wordsElements.forEach((el) => {
    wordIndex = 0; // Reset per container
    recursiveSplit(el as HTMLElement, 'words');
    (el as HTMLElement).style.setProperty('--tw-total-splits', `${wordIndex}`);
  });
};
